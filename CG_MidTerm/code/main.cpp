...
double deltatime=1.0;		// 间隔时间，初始设为1s
...
glm::vec2 allRotationsLshape[7][4][4] =
{
	...
	{ {glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(-1, -1), glm::vec2(0, -1)},	//   "O"
		{glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(-1, -1), glm::vec2(0, -1)},
		{glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(-1, -1), glm::vec2(0, -1)},
		{glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(-1, -1), glm::vec2(0, -1)}},

	{ {glm::vec2(-2, 0), glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(1, 0)},	//   "I"
		{ glm::vec2(0, 1), glm::vec2(0, 0), glm::vec2(0,-1), glm::vec2(0, -2) },   //	 
		{ glm::vec2(-2, 0), glm::vec2(0, 0), glm::vec2(-1, 0), glm::vec2(1, 0) },   //
		{ glm::vec2(0, 1), glm::vec2(0, 0), glm::vec2(0,-1), glm::vec2(0, -2)}},

	{ {glm::vec2(0, 0), glm::vec2(0, -1), glm::vec2(1, 0), glm::vec2(-1, -1)},	//   "S"
...
glm::vec4 pink	 = glm::vec4(1.0, 0.0, 1.0, 1.0);
glm::vec4 colorType[] = {orange,blue,red,green,yellow,pink};
int randColorNum;
...
// （以棋盘格的左下角为原点的坐标系）
bool board[board_width][board_height+1];

// 当棋盘格某些位置被方块填充之后，记录这些位置上被填充的颜色
glm::vec4 board_colours[points_num];
glm::vec4 boardColor[board_width][board_height+1];
...
void newtile()
{
	...
	//生成新方块时，判断是否与已填入的方块重叠，如果是-游戏结束
	for (int i = 0; i < 4; i++)
	{
		tile[i] = allRotationsLshape[shapeType][0][i];
	...
	randColorNum = rand() % 6;
	for (int i = 0; i < 24; i++)
		newcolours[i] = colorType[randColorNum];
...
void init()
{
	...
	std::cout << "KEY_UP--rotate; KEY_DOWN--move down;\n\
KEY_LEFT--move left; KEY_RIGHT--move right" << std::endl;
	std::cout << "q or esc -- quit ; r -- restart" << std::endl;
...
void rotate()
{
	...
	// 检查当前旋转之后的位置的有效性
	if (checkvalid((allRotationsLshape[shapeType][nextrotation][0]) + tilepos)
		&& checkvalid((allRotationsLshape[shapeType][nextrotation][1]) + tilepos)
		&& checkvalid((allRotationsLshape[shapeType][nextrotation][2]) + tilepos)
		&& checkvalid((allRotationsLshape[shapeType][nextrotation][3]) + tilepos))
	{
		// 更新旋转，将当前方块设置为旋转之后的方块
		rotation = nextrotation;
		for (int i = 0; i < 4; i++)
			tile[i] = allRotationsLshape[shapeType][rotation][i];
...
void checkfullrow(int row)
{
	...
	if (flag) {
		...
		for (int i = 0; i < board_width; ++i) {
			board[i][r] = false;
			changecellcolour(glm::vec2(i, r), black);
			boardColor[i][r] = black;
		}
		for (int i = 0; i < board_width; ++i) {
			for (int j = r; j < board_height ; ++j) {
				board[i][j] = board[i][j + 1];
				changecellcolour(glm::vec2(i, j), boardColor[i][j+1]);
				board[i][j + 1] = false;
				boardColor[i][j + 1] = black;
			}
		}
		//修改分数，并且根据分数改变物块下落间隔时间
		Score += board_width;
		if (Score<10) {
...
void settile()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		board[x][y] = true;
		// 并将相应位置的颜色修改
		changecellcolour(glm::vec2(x, y), colorType[randColorNum]);
	}
	//检查四个格子所在的行是否填满
	for (int i = 0; i < 4; i++) {
		int y = (tile[i] + tilepos).y;
...
int main(int argc, char **argv)
{
	...
	// 创建窗口。
	GLFWwindow* window = glfwCreateWindow(500, 900, "2020284022_吴洪涛_期中作业", NULL, NULL);
	if (window == NULL)
	{
    ...
    { 
		...
		//定时器功能，每隔deltatime时间，物块下移
		double nowtime = glfwGetTime();
		if (nowtime - previoustime >= deltatime) {
			previoustime = nowtime;
			if (!gameover&&!movetile(glm::vec2(0, -1)))
			{
				settile();
				newtile();
			}
