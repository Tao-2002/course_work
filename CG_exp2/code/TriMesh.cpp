...
void TriMesh::storeFacesPoints() {
    ...
    for (int i = 0; i < faces.size(); i++) {
        ...
        //同一面统一颜色
        //colors.push_back(vertex_colors[i%8]);
        //colors.push_back(vertex_colors[i%8]);
        //colors.push_back(vertex_colors[i%8]);
        // 
        //colors.push_back(basic_colors[i%8]);
        //colors.push_back(basic_colors[i%8]);
        //colors.push_back(basic_colors[i%8]);
    }
...
void TriMesh::generateCube() {
    ...
    faces.push_back(vec3i(4, 1, 5));
    faces.push_back(vec3i(7, 6, 5));
...
void TriMesh::readOff(const std::string& filename)
{
    ...
    else
    {
        ...
        for (int i = 0; i < nVertices; i++)
        {
			...
			vertex_colors.push_back(glm::vec3(tmp_node.x+0.3,tmp_node.y+0.3,tmp_node.z+0.3));
        }
