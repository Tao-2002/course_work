...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	//原点
	vertex_positions.push_back(glm::vec3(0.0, 0, 0.0));
	vertex_normals.push_back(normalize(glm::vec3(0.0, 0, 0.0)));
	vertex_colors.push_back(normalize(glm::vec3(0.0, 0, 0.0)));

	for (int i = 0; i < num_samples; i++) {
		faces.push_back(vec3i(i, (i + 1) % num_samples, num_samples));
		float r_r_r = i * step;
		float x = (float)(0.50 + radius * cos(r_r_r));
		float y = (float)(0.50 + radius * sin(r_r_r));
		vertex_textures.push_back(glm::vec2(x, y));
		float next_r_r_r = ((i + 1) % num_samples) * step;
		float next_x = (float)(0.5 + radius * cos(next_r_r_r));
		float next_y = (float)(0.5 + radius * sin(next_r_r_r));
		vertex_textures.push_back(glm::vec2(next_x, next_y));
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	for (int i = 0; i < num_samples; i++) {
		...
		vertex_positions.push_back(glm::vec3(x, y, z));
		vertex_normals.push_back(normalize(glm::vec3(x, y, z)));
		vertex_colors.push_back(normalize(glm::vec3(x, y, z)));
	}
	z = -height / 2;
	vertex_positions.push_back(glm::vec3(0, 0, z));
	vertex_normals.push_back(normalize(glm::vec3(0, 0, z)));
	vertex_colors.push_back(normalize(glm::vec3(0, 0, z)));
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		vertex_textures.push_back(glm::vec2(0.5, 0.99));
...
void TriMesh::readObj(const std::string& filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		std::string type;
		GLfloat _x=0.0, _y=0.0, _z=0.0;
		int a0, b0, c0;
		int a1, b1, c1;
		int a2, b2, c2;
		char slash;
		// @TODO: Task2 读取obj文件，记录里面的这些数据，可以参考readOff的写法
		// vertex_positions vertex_normals	vertex_textures	
	...
	//std::cout << "faces:" << faces.size() << " vertex_positions:" << vertex_positions.size() 
		//<< " vertex_normals:" << vertex_normals.size() <<" vertex_textures:"<< vertex_textures.size() <<std::endl;
	fin.close();
