...
// #endif
	...
	modelView = glm::rotate(modelView, glm::radians(Theta[UpperArm]), glm::vec3(0, 1, 0));
	upper_arm(modelView);
	// 绘制大臂

	// 小臂变换矩阵
	modelView = glm::translate(modelView, glm::vec3(0, UPPER_ARM_HEIGHT, 0));
	modelView = glm::rotate(modelView, glm::radians(Theta[LowerArm]), glm::vec3(0, 1, 0));
	lower_arm(modelView);
	// 绘制小臂	
...
int main(int argc, char **argv)
{
	...
	// 配置窗口属性
	GLFWwindow* window = glfwCreateWindow(600, 600, "2020284022_吴洪涛_补充实验1", NULL, NULL);
	if (window == NULL)
	{
